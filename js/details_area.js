...
console.log(showAreaDetails(id))

function displayareaDetails(info) {
  let cartona = ` <div class="col-md-4">
                <img class="w-100 rounded-3" src=${info.strMealThumb} alt="">
                <h3 class ="text-white">${info.strMeal}</h3>
            </div>
            <div class="col-md-8 text-white fs-5 ms-5">
                <div class="ins mt-5 ">
                    <h3>Instraction</h3>
                <p class="fs-6">${info.strInstructions.split(" ",100).join(" ")}</p>
                </div>
                <div class="reci">
                   <h4>Area:<span>${info.strArea}</span></h4> 
                   <h4>Category:<span>${info.strCategory}</span></h4> 
                  <div>
                    <h4>recipe:</h4>
                  ...
                  <h4>tags:</h4>
                  <a href=${info.strSource} class="btn btn-success">Source</a>
                  <a href=${info.strYoutube} class="btn btn-danger">Youtube</a>
